...
import type { ActivityRow } from '@mappers/activity/genericActivityMappers';
import { DetailCard } from '@features/sales/screens/orderDetail/OrderDetailPrimitives';

export type ModelActivityLogPanelProps = {
  modelType: string;
  modelId: number;
  reloadSignal?: string | number;
  /** e.g. "customers.detail" — uses activityTitle, activityLoading, … */
  i18nPrefix: string;
  toRow: (item: ActivityLogApi) => ActivityRow;
};
...
export function ModelActivityLogPanel({
  modelType,
  modelId,
  reloadSignal = 0,
  i18nPrefix,
  toRow,
}: ModelActivityLogPanelProps) {
  ...
  }, [items, toRow, i18nInstance.language]);

  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const res = await getModelActivities(modelType, modelId, {
        per_page: 50,
      });
      setItems(res.items);
    } catch (e: unknown) {
      ...
      setError(
        e instanceof Error
          ? e.message
          : t(`${i18nPrefix}.activityLoadFailed`),
      );
    } finally {
      setLoading(false);
    }
  }, [modelType, modelId, i18nPrefix, t]);
  ...
  return (
    <DetailCard title={t(`${i18nPrefix}.activityTitle`)} icon="activity">
      {loading ? (
        <View style={styles.centerBox}>
          <ActivityIndicator size="small" color={palette.tealLight} />
          <Text style={styles.loadingTxt}>
            {t(`${i18nPrefix}.activityLoading`)}
          </Text>
        </View>
      ) : null}
      ...
      {!loading && !error && rows.length === 0 ? (
        <View style={styles.emptyInner}>
          ...
          <Text style={styles.emptyTitle}>
            {t(`${i18nPrefix}.activityEmptyTitle`)}
          </Text>
          <Text style={styles.emptyHint}>
            {t(`${i18nPrefix}.activityEmptyHint`)}
          </Text>
        </View>
      ) : null}
