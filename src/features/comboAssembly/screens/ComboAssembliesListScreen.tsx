...
import {
  ComboAssembliesFilterToolbar,
  type ComboAssemblyListPreset} from '../components/ComboAssembliesFilterToolbar';
import { CreateComboAssemblyModal } from '../components/CreateComboAssemblyModal';
import { ComboAssemblyListMobileCard } from '../components/ComboAssemblyListMobileCard';
import { comboAssemblyToListRow } from '@mappers/warehouse/comboAssemblyMappers';
import type { ComboAssemblyListRow } from '../comboAssemblyTypes';
import { getComboAssembliesPage } from '@services/warehouse/comboAssemblyAPI';
import type { ComboAssemblyApi } from '@services/warehouse/comboAssemblyApiTypes';

const MAX_WINDOW = 100;
const PER_PAGE = 20;

export type ComboAssembliesListScreenProps = {
  onOpenDrawer: () => void;
...
function presetToApiParams(p: ComboAssemblyListPreset): {
  filterStatus?: string;
} {
  if (p === 'all') return {};
...
export function ComboAssembliesListScreen({
  onOpenDrawer,
  onOpenComboAssembly}: ComboAssembliesListScreenProps) {
  const palette = useAppColors();
  const styles = useThemeStyleSheet(create_ComboAssembliesListScreen_styles);
  ...
  const [items, setItems] = useState<ComboAssemblyApi[]>([]);
  ...
  const [preset, setPreset] = useState<ComboAssemblyListPreset>('all');
  ...
  const fetchPage = useCallback(async (targetPage: number, append: boolean) => {
    ...
    try {
      ...
      const data = await getComboAssembliesPage({
        page: targetPage,
        per_page: PER_PAGE,
        filterStatus,
  ...
  const rows: ComboAssemblyListRow[] = useMemo(
    () => items.map(comboAssemblyToListRow),
    [items],
  );

  const onRow = useCallback(
    (row: ComboAssemblyListRow) => {
      if (onOpenComboAssembly) {
        onOpenComboAssembly({
          uuid: row.uuid,
      ...
      toast.info(`${row.productName}\n${row.warehouseName}\nSL ${row.quantityLabel}`);
    },
    [onOpenComboAssembly],
  );

  const keyExtractor = useCallback((row: ComboAssemblyListRow) => String(row.id), []);

  const renderItem = useCallback(
    ({ item }: { item: ComboAssemblyListRow }) => (
      <ComboAssemblyListMobileCard row={item} onPress={() => onRow(item)} />
    ),
    [onRow],
  );

  const listFilterActive = preset !== 'all' || query.trim().length > 0;
      ...
      <>
        ...
        <CanvasListToolbarShell>
          <ComboAssembliesFilterToolbar
            ...
            primaryActionTitle="+ Tạo yêu cầu mới"
            onPrimaryAction={() => setCreateOpen(true)}
          />
        </CanvasListToolbarShell>
        <View style={styles.listHeaderRow}>
          <SectionTitle label="Đóng gói combo" />
        ...
        {rows.length === 0 && !loading ? (
          <Text style={[styles.emptyText, { color: palette.textMuted }]}>
            Không có lệnh lắp combo phù hợp.
          </Text>
        ) : null}
      </>
    ),
    [
      error, query, preset, listFilterActive, clearListFilters, onRefresh, refreshing,
      itemCount, rows.length, loading, palette, styles,
    ],
  );
...
function create_ComboAssembliesListScreen_styles(c: AppColorPalette) {
