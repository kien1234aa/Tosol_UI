...
import {
  PackingOrdersFilterToolbar,
  type PackingOrderListPreset} from '../components/PackingOrdersFilterToolbar';
import { PackingOrderListMobileCard } from '../components/PackingOrderListMobileCard';
import { packingOrderToListRow } from '@mappers/warehouse/packingMappers';
import type { PackingOrderListRow } from '../packingTypes';
import { getPackingOrdersPage } from '@services/warehouse/packingOrderAPI';
import type { PackingOrderApi } from '@services/warehouse/packingOrderApiTypes';

const MAX_WINDOW = 100;
const PER_PAGE = 20;

export type PackingOrdersListScreenProps = {
  onOpenDrawer: () => void;
...
function presetToApiParams(p: PackingOrderListPreset): {
  filterStatus?: string;
  filterHasPendingUnpack?: boolean;
} {
  if (p === 'all') return {};
  if (p === 'unpack') return { filterHasPendingUnpack: true };
  return { filterStatus: p };
}

export function PackingOrdersListScreen({
  onOpenDrawer,
  onOpenPackingOrder}: PackingOrdersListScreenProps) {
  const palette = useAppColors();
  const styles = useThemeStyleSheet(create_PackingOrdersListScreen_styles);
  ...
  const [items, setItems] = useState<PackingOrderApi[]>([]);
  ...
  const [preset, setPreset] = useState<PackingOrderListPreset>('all');
  ...
  const fetchPage = useCallback(async (targetPage: number, append: boolean) => {
    ...
    try {
      const { filterStatus, filterHasPendingUnpack } = presetToApiParams(preset);
      const data = await getPackingOrdersPage({
        page: targetPage,
        per_page: PER_PAGE,
        filterStatus,
      ...
      if (!append) {
        setItems([]);
        setListMeta(null);
        setError(e instanceof Error ? e.message : 'Không tải được lệnh đóng gói');
      }
    } finally {
      if (append) {
  ...
  const rows: PackingOrderListRow[] = useMemo(
    () => items.map(packingOrderToListRow),
    [items],
  );
  ...
  const onRow = useCallback(
    (row: PackingOrderListRow) => {
      if (onOpenPackingOrder) {
        onOpenPackingOrder({ uuid: row.uuid, orderNumber: row.orderNumber, id: row.id });
        return;
      }
      toast.info(`Đơn bán: ${row.saleOrderNumber}\n${row.warehouseName}`);
    },
    [onOpenPackingOrder],
  );

  const keyExtractor = useCallback((row: PackingOrderListRow) => String(row.id), []);

  const renderItem = useCallback(
    ({ item }: { item: PackingOrderListRow }) => (
      <PackingOrderListMobileCard row={item} onPress={() => onRow(item)} />
    ),
    [onRow],
  );
    ...
    () => (
      <>
        ...
        <View style={styles.listHeaderRow}>
          <SectionTitle label="Lệnh đóng gói" />
        ...
        {rows.length === 0 && !loading ? (
          <Text style={[styles.emptyText, { color: palette.textMuted }]}>
            Không có lệnh đóng gói phù hợp.
          </Text>
        ) : null}
      </>
    ),
...
function create_PackingOrdersListScreen_styles(c: AppColorPalette) {
