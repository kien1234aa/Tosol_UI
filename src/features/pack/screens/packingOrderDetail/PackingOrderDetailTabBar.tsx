...
import { PACKING_ORDER_DETAIL_TABS } from './packingOrderDetailTypes';

export type PackingOrderDetailTabBarProps = {
  tabs?: typeof PACKING_ORDER_DETAIL_TABS;
  activeTab: PackingOrderDetailTabId;
  boxCount: number;
  onSelectTab: (id: PackingOrderDetailTabId) => void;
};

export function PackingOrderDetailTabBar({
  tabs = PACKING_ORDER_DETAIL_TABS,
  activeTab,
  boxCount,
  onSelectTab,
}: PackingOrderDetailTabBarProps) {
  const styles = useThemeStyleSheet(create_PackingOrderDetailTabBar_styles);
    ...
    <View style={styles.tabBarOuter}>
      ...
      >
        {tabs.map(tab => {
          ...
          const badge =
            tab.badgeFrom === 'boxes' && boxCount > 0 ? String(boxCount) : null;
          return (
            <Pressable
              key={tab.id}
...
function create_PackingOrderDetailTabBar_styles(c: AppColorPalette) {
