...
import {
  TransferOrdersFilterToolbar,
  type TransferOrderListPreset} from '../components/TransferOrdersFilterToolbar';
import { TransferOrderListMobileCard } from '../components/TransferOrderListMobileCard';
import { transferOrderToListRow } from '@mappers/warehouse/transferOrderMappers';
import type { TransferOrderListRow } from '../transferOrderTypes';
import { getTransferOrdersPage } from '@services/warehouse/transferOrderAPI';
import type { TransferOrderApi } from '@services/warehouse/transferOrderApiTypes';

const MAX_WINDOW = 100;
const PER_PAGE = 20;

export type TransferOrdersListScreenProps = {
  onOpenDrawer: () => void;
...
function presetToApiParams(p: TransferOrderListPreset): {
  filterStatus?: string;
} {
  if (p === 'all') return {};
...
export function TransferOrdersListScreen({
  onOpenDrawer,
  onOpenTransferOrder}: TransferOrdersListScreenProps) {
  const palette = useAppColors();
  const styles = useThemeStyleSheet(create_TransferOrdersListScreen_styles);
  ...
  const [items, setItems] = useState<TransferOrderApi[]>([]);
  ...
  const [preset, setPreset] = useState<TransferOrderListPreset>('all');
  ...
  const fetchPage = useCallback(async (targetPage: number, append: boolean) => {
    ...
    try {
      ...
      const data = await getTransferOrdersPage({
        page: targetPage,
        per_page: PER_PAGE,
        filterStatus,
      ...
      if (!append) {
        setItems([]);
        setListMeta(null);
        setError(e instanceof Error ? e.message : 'Không tải được lệnh chuyển kho');
      }
    } finally {
      if (append) {
  ...
  const rows: TransferOrderListRow[] = useMemo(
    () => items.map(transferOrderToListRow),
    [items],
  );
  ...
  const onRow = useCallback(
    (row: TransferOrderListRow) => {
      if (onOpenTransferOrder) {
        onOpenTransferOrder({ uuid: row.uuid, orderNumber: row.orderNumber, id: row.id });
        return;
      }
      toast.info(`${row.routeLabel}\n${row.scanProgressLabel}`);
    },
    [onOpenTransferOrder],
  );

  const keyExtractor = useCallback((row: TransferOrderListRow) => String(row.id), []);

  const renderItem = useCallback(
    ({ item }: { item: TransferOrderListRow }) => (
      <TransferOrderListMobileCard row={item} onPress={() => onRow(item)} />
    ),
    [onRow],
  );
    ...
    () => (
      <>
        ...
        <View style={styles.listHeaderRow}>
          <SectionTitle label="Lệnh chuyển kho" />
        ...
        {rows.length === 0 && !loading ? (
          <Text style={[styles.emptyText, { color: palette.textMuted }]}>
            Không có lệnh chuyển kho phù hợp.
          </Text>
        ) : null}
      </>
    ),
...
function create_TransferOrdersListScreen_styles(c: AppColorPalette) {
