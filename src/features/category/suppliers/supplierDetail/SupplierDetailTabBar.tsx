...
import { SUPPLIER_DETAIL_TABS } from './supplierDetailTypes';
import type { SupplierDetailTabId } from './supplierDetailTypes';

export type SupplierDetailTabBarProps = {
  tabs?: typeof SUPPLIER_DETAIL_TABS;
  activeTab: SupplierDetailTabId;
  purchaseOrdersCount: number;
  onSelectTab: (id: SupplierDetailTabId) => void;
};

export function SupplierDetailTabBar({
  tabs = SUPPLIER_DETAIL_TABS,
  activeTab,
  purchaseOrdersCount,
  onSelectTab,
}: SupplierDetailTabBarProps) {
  const styles = useThemeStyleSheet(create_SupplierDetailTabBar_styles);
    ...
    <View style={styles.tabBarOuter}>
      ...
      >
        {tabs.map(tab => {
          ...
          const badge =
            tab.id === 'orders' && purchaseOrdersCount > 0
              ? String(purchaseOrdersCount)
              : null;
          return (
            <Pressable
              key={tab.id}
...
function create_SupplierDetailTabBar_styles(c: AppColorPalette) {
