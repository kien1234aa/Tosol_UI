...
import type { ProductDetailTabId } from './productDetailTypes';
import { PRODUCT_DETAIL_TABS } from './productDetailTypes';

export type ProductDetailTabBarProps = {
  tabs?: typeof PRODUCT_DETAIL_TABS;
  activeTab: ProductDetailTabId;
  imageCount: number;
  onSelectTab: (id: ProductDetailTabId) => void;
};

export function ProductDetailTabBar({
  tabs = PRODUCT_DETAIL_TABS,
  activeTab,
  imageCount,
  onSelectTab,
}: ProductDetailTabBarProps) {
  const styles = useThemeStyleSheet(create_ProductDetailTabBar_styles);
    ...
    <View style={styles.tabBarOuter}>
      ...
      >
        {tabs.map(tab => {
          ...
          const badge =
            tab.id === 'images' && imageCount > 0 ? String(imageCount) : null;
          return (
            <Pressable
              key={tab.id}
...
function create_ProductDetailTabBar_styles(c: AppColorPalette) {
