...
import { CUSTOMER_DETAIL_TABS } from './customerDetailTypes';
import type { CustomerDetailTabId } from './customerDetailTypes';

export type CustomerDetailTabBarProps = {
  tabs?: typeof CUSTOMER_DETAIL_TABS;
  activeTab: CustomerDetailTabId;
  saleOrdersCount: number;
  onSelectTab: (id: CustomerDetailTabId) => void;
};
...
export function CustomerDetailTabBar({
  ...
  const styles = useThemeStyleSheet(create_CustomerDetailTabBar_styles);
    ...
    <View style={styles.tabBarOuter}>
      ...
      >
        {tabs.map(tab => {
          ...
          const badge =
            tab.id === 'orders' && saleOrdersCount > 0
              ? String(saleOrdersCount)
              : null;
          return (
            <Pressable
              key={tab.id}
...
function create_CustomerDetailTabBar_styles(c: AppColorPalette) {
