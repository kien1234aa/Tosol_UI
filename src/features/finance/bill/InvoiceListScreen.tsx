...
import {
  ...
  setInvoiceListSearch} from '@services/finance/invoiceSlice';
import { InvoiceFilterToolbar } from './components/InvoiceFilterToolbar';
import { InvoiceListMobileCard } from './components/InvoiceListMobileCard';
import { invoiceApiToListRow } from '@mappers/finance/invoiceListMappers';

export type InvoiceListScreenProps = {
  onOpenDrawer: () => void;
...
export function InvoiceListScreen({
  onOpenDrawer,
  onOpenInvoice}: InvoiceListScreenProps) {
  const insets = useSafeAreaInsets();
  const palette = useAppColors();
  const styles = useThemeStyleSheet(create_InvoiceListScreen_styles);
  ...
  useEffect(() => {
    const t = setTimeout(() => {
      ...
      if (next !== listSearch) {
        dispatch(setInvoiceListSearch(next));
      }
    }, 350);
  ...
  useEffect(() => {
    void dispatch(fetchInvoices({ page: 1 }));
  }, [dispatch, listFilter, listSearch]);

  useEffect(() => {
    void dispatch(fetchInvoiceCounts());
  }, [dispatch]);

  const rows = useMemo(() => items.map(invoiceApiToListRow), [items]);
  ...
  const handleLoadMore = useCallback(() => {
    ...
    void dispatch(fetchInvoices({ page: currentPage + 1, append: true }));
  }, [canLoadMore, currentPage, dispatch]);

  const listFilterActive = listFilter !== 'all' || query.trim().length > 0;

  const clearListFilters = useCallback(() => {
    dispatch(clearInvoiceListFilter());
    dispatch(setInvoiceListSearch(''));
    setQuery('');
  }, [dispatch]);

  type RowItem = ReturnType<typeof invoiceApiToListRow>;
  ...
  const renderItem = useCallback(
    ({ item }: { item: RowItem }) => (
      <InvoiceListMobileCard
        row={item}
        onPress={
          onOpenInvoice != null ? () => onOpenInvoice(item.id) : undefined
        }
      />
    ),
    [onOpenInvoice],
      ...
      <View>
        ...
        <CanvasListToolbarShell>
          <InvoiceFilterToolbar
            ...
            filter={listFilter}
            onFilterChange={fl => dispatch(setInvoiceListFilter(fl))}
...
function create_InvoiceListScreen_styles(c: AppColorPalette) {
