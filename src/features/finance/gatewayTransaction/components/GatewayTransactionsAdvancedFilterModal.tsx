...
import { getSellersPage } from '@services/settings/adminSellersAPI';
import {
  GATEWAY_TXN_STATUS_OPTIONS,
  GATEWAY_TXN_TYPE_OPTIONS,
} from '../gatewayTxnListFilterOptions';
import type {
...
export type GatewayTxnAdvancedFilterApply = {
  status: GatewayTxnListFilter;
  sellerId: string;
  type: GatewayTxnListTypeFilter;
  dateFrom: string;
  dateTo: string;
};

export type GatewayTransactionsAdvancedFilterModalProps = {
  visible: boolean;
  statusFilter: GatewayTxnListFilter;
  sellerId: string;
  typeFilter: GatewayTxnListTypeFilter;
  dateFrom: string;
  dateTo: string;
  onClose: () => void;
  onApply: (next: GatewayTxnAdvancedFilterApply) => void;
};
...
function countActive(d: GatewayTxnAdvancedFilterApply): number {
  let n = 0;
  if (d.sellerId.trim()) {
    n += 1;
  }
...
const emptyDraft: GatewayTxnAdvancedFilterApply = {
  status: 'all',
  sellerId: '',
  type: 'all',
  dateFrom: '',
...
export function GatewayTransactionsAdvancedFilterModal({
  visible,
  statusFilter,
  sellerId: appliedSellerId,
  typeFilter: appliedTypeFilter,
  dateFrom: appliedDateFrom,
  dateTo: appliedDateTo,
  onClose,
  onApply,
}: GatewayTransactionsAdvancedFilterModalProps) {
  const palette = useAppColors();
  ...
  const [draft, setDraft] = useState<GatewayTxnAdvancedFilterApply>(() => ({
    status: statusFilter,
    sellerId: appliedSellerId,
  ...
  const [picker, setPicker] = useState<null | 'seller' | 'status' | 'type'>(
    null,
  );
  const [sellerOpts, setSellerOpts] = useState<SelectOpt[]>([
    { label: 'Tất cả', value: '' },
  ]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const { items } = await getSellersPage({
          per_page: 150,
          isActive: true,
          signal: ac.signal,
        });
        ...
        setSellerOpts([
          ...
          ...items.map(i => ({
            value: String(i.id),
            label: i.name?.trim() || i.code || String(i.id),
          })),
        ]);
      } catch {
        if (!cancelled) {
          setSellerOpts([{ label: 'Tất cả', value: '' }]);
        }
      }
    })();
    return () => {
  ...
  const statusOpts = useMemo(
    () => toSelectOpts(GATEWAY_TXN_STATUS_OPTIONS),
    [],
  );
  const typeOpts = useMemo(() => toSelectOpts(GATEWAY_TXN_TYPE_OPTIONS), []);

  const statusLabel = useMemo(
    () => labelForKey(GATEWAY_TXN_STATUS_OPTIONS, draft.status),
    [draft.status],
  );
  const typeLabel = useMemo(
    () => labelForKey(GATEWAY_TXN_TYPE_OPTIONS, draft.type),
    [draft.type],
  );
  const sellerLabel = useMemo(() => {
    const v = draft.sellerId.trim();
    if (!v) {
      return 'Tất cả';
    }
    return sellerOpts.find(o => o.value === v)?.label ?? v;
  }, [draft.sellerId, sellerOpts]);
  ...
  const pickerOpts = useMemo(() => {
    ...
    return [];
  }, [picker, sellerOpts, statusOpts, typeOpts]);
      ...
      <FilterSheetHostProvider value>
        <View style={st.root}>
                ...
                <View style={st.headerTitleRow}>
                  ...
                  <View style={st.headerTitles}>
                    ...
                    <Text style={st.headerSub} numberOfLines={2}>
                      Nhà cung cấp, trạng thái, loại và khoảng thời gian.
                    </Text>
                  </View>
                </View>
              </View>
          ...
          <OptionsModal
            ...
            onSelect={v => {
              if (picker === 'seller') {
                setDraft(prev => ({ ...prev, sellerId: v }));
              } else if (picker === 'status') {
                setDraft(prev => ({
                  ...prev,
                  status: v as GatewayTxnListFilter,
                }));
              } else if (picker === 'type') {
                setDraft(prev => ({
                  ...prev,
                  type: v as GatewayTxnListTypeFilter,
                }));
              }
            }}
