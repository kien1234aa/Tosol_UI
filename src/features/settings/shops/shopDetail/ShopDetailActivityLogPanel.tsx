...
import { shopActivityToRow } from '@mappers/settings/shopActivityMappers';
import { DetailCard } from '../../../sales/screens/orderDetail/OrderDetailPrimitives';

export type ShopDetailActivityLogPanelProps = {
  shopId: number;
...
function ActivityTimelineItem({
  ...
  row: ReturnType<typeof shopActivityToRow>;
  isFirst: boolean;
  isLast: boolean;
...
export function ShopDetailActivityLogPanel({
  shopId,
  reloadSignal = 0,
}: ShopDetailActivityLogPanelProps) {
  ...
  const [rows, setRows] = useState<ReturnType<typeof shopActivityToRow>[]>([]);

  useEffect(() => {
    setRows(items.map(shopActivityToRow));
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await getModelActivities('Shop', shopId, {
        per_page: 50,
      });
      setItems(res.items);
    } catch (e: unknown) {
      ...
      setError(
        e instanceof Error
          ? e.message
          : t('shops.detail.activityLoadFailed'),
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <DetailCard title={t('shops.detail.activityTitle')} icon="activity">
      {loading ? (
        <View style={styles.centerBox}>
          <ActivityIndicator size="small" color={palette.tealLight} />
          <Text style={styles.loadingTxt}>
            {t('shops.detail.activityLoading')}
          </Text>
        </View>
      ) : null}
      ...
      {!loading && !error && rows.length === 0 ? (
        <View style={styles.emptyInner}>
          ...
          <Text style={styles.emptyTitle}>
            {t('shops.detail.activityEmptyTitle')}
          </Text>
          <Text style={styles.emptyHint}>
            {t('shops.detail.activityEmptyHint')}
          </Text>
        </View>
      ) : null}
