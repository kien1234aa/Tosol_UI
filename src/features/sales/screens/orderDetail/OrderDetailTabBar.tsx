...
import { ORDER_TABS } from './orderDetailTypes';

export type OrderDetailTabBarProps = {
  /** Danh sách tab theo policy (`orderDetailTabsForAppRole`). */
  tabs?: typeof ORDER_TABS;
  activeTab: OrderDetailTabId;
  itemCount: number;
  onSelectTab: (id: OrderDetailTabId) => void;
};

export function OrderDetailTabBar({
  tabs = ORDER_TABS,
  activeTab,
  itemCount,
  onSelectTab,
}: OrderDetailTabBarProps) {
  const styles = useThemeStyleSheet(create_OrderDetailTabBar_styles);
...
function create_OrderDetailTabBar_styles(c: AppColorPalette) {
